...
#transform columns where they are are a character class to lowercase 
```
...
ggplot(data = monroe_ts, aes(x = month_name, 
                             y = total_k_wh)) +
  geom_jitter() 
```
...
ggsave(here("fig","graph_c_gc.png"), width = 5, height = 6)
```
